...
class ApiController extends Controller
{
    ...
    public function resentPinApp(Request $request)
    {
        ...
        if(count($user) > 0) {
            ...
            if(count($user_meta) > 0) {
                ...
                $response["errors"]= [
                    "statusCode"=> 404,
                    "errorMessage"=> "Pin number not found",
                    "serverReferenceCode"=> $now
                ];
            }
        } else {
    ...
    public function AppRegistration(Request $request)
    {
        ...
        if(count($existing_user) > 0) {
            $response["success"]= [
                ...
                "user_mobile" => $existing_user->user_mobile
            ];
        } else {
            ...
            try {
                ...
                $response["errors"]= [
                    "statusCode"=> 501,
                    "errorMessage"=> "Email or mobile number already exist",
                    "serverReferenceCode"=> $now,
                    'user_info' => $userinfo,
                ];
                $message = "Message : ".$e->getMessage().", File : ".$e->getFile().", Line : ".$e->getLine();
    ...
    public function AppPinConfirm(Request $request)
    {
        ...
        if(count($user) > 0) {
            try {
                ...
                if(count($user_meta) > 0) {
                    ...
                    if($sent_pin == $pin_confirm) {
                        ...
                        $response["errors"]= [
                            "statusCode"=> 504,
                            "errorMessage"=> "Pin number not match",
                            "serverReferenceCode"=> $now,
                        ];
                        return response()->json($response);
                    }
                    ...
                    $response["errors"]= [
                        "statusCode"=> 503,
                        "errorMessage"=> "Pin number not valid",
                        "serverReferenceCode"=> $now,
                    ];
                    return response()->json($response);
                }
                ...
                $response["errors"]= [
                    "statusCode"=> 502,
                    "errorMessage"=> "Something went wrong, Sorry the requested resource does not exist",
                    "serverReferenceCode"=> $now,
                ];
                $message = "Message : ".$e->getMessage().", File : ".$e->getFile().", Line : ".$e->getLine();
                \App\System::ErrorLogWrite($message);
            ...
            $response["errors"]= [
                "statusCode"=> 501,
                "errorMessage"=> "User not found ! Please registration first.",
                "serverReferenceCode"=> $now,
            ];
            return response()->json($response);
        }
    ...
    public function getUserActivities(Request $request)
    {
        ...
        if(count($activities) > 0) {
            ...
            $response["errors"]= [
                "statusCode"=> 501,
                "errorMessage"=> "User activities not found",
                "serverReferenceCode"=> $now,
            ];
            return response()->json($response);
        }
    ...
    public function appProfileUpdate(Request $request)
    {
        ...
        if(count($user) > 0) {
            ...
            try {
                ...
                $response["errors"]= [
                    "statusCode"=> 502,
                    "errorMessage"=> "Profile updated not successfully",
                    "serverReferenceCode"=> $now,
                ];
                return response()->json($response);
            }
